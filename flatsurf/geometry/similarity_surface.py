...
def combine_surface_types(type1, type2):
    ...
    if type1==SurfaceType.SIMILARITY or type2==SurfaceType.SIMILARITY:
        return SurfaceType.SIMILARITY
    
    # Deal with the case when one is a half-translation.
    if type1==SurfaceType.HALF_TRANSLATION:
        if type2 == SurfaceType.DILATION:
            return SurfaceType.HALF_DILATION
        return type2
    if type2==SurfaceType.HALF_TRANSLATION:
        if type1 == SurfaceType.DILATION:
            return SurfaceType.HALF_DILATION
        return type1
    ...
    if type1==SurfaceType.DILATION:
        ...
        return type2
    if type2==SurfaceType.DILATION:
        if type1==SurfaceType.CONE or type1==SurfaceType.RATIONAL_CONE:
            return SurfaceType.SIMILARITY
        return type1
    
    # Now if one is Half-Dilation, then the combination must be Similarity
    # since we ruled out equality of types.
    if type1==SurfaceType.HALF_DILATION or type2==SurfaceType.HALF_DILATION:
        return SurfaceType.SIMILARITY
