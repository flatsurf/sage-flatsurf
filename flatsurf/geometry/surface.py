    ...
    def ramified_cover(self, d, data):
        ...
        for i in range(1, d + 1):
            for lab in self.label_iterator():
                cover_labels[(lab, i)] = cover.add_polygon(
                    self.polygon(lab), label=cover.num_polygons()
                )
        while edges:
...
class Surface_list(Surface):
    ...
    def __init__(self, base_ring=None, surface=None, copy=None, mutable=None):
        ...
        warnings.warn(
            "Surface_list is deprecated and will be removed from a future version of sage-flaturf. Use Surface_dict instead."
        )
...
def surface_list_from_polygons_and_gluings(polygons, gluings, mutable=False):
    ...
    warnings.warn(
        "surface_list_from_polygons_and_gluings() is deprecated and will be removed from a future version of sage-flatsurf. Build a Surface_dict explicitly instead."
    )
...
class Surface_dict(Surface):
    ...
    def _add_polygon(self, new_polygon, gluing_list=None, label=None):
        ...
        if label is None:
            ...
            warnings.warn(
                "calling add_polygon() without a label is deprecated and will be an error in future versions of sage-flatsurf. Explicitly specify a label such as .num_polygons()."
            )
...
class ExtraLabel(SageObject):
    ...
    def __init__(self, value=None, nowarn=False):
        ...
        if not nowarn:
            ...
            warnings.warn(
                "ExtraLabel is deprecated and will be removed in a future version of sage-flatsurf. Please use a unique label instead."
            )
