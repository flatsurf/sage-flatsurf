...
class SplitPolygonsMapping(SurfaceMapping):
    ...
    def __init__(self, s, p, v1, v2, new_label=None):
        ...
        if new_label is None:
            ...
            warnings.warn(
                "creating a SplitPolygonsMapping without a new_label is deprecated. Speficy a new_label explicitly, e.g., new_label='split'"
            )
...
def subdivide_a_polygon(s, new_label=None):
    ...
    if new_label is None:
        ...
        warnings.warn(
            "calling subdivide_a_polygon() without specficying a new_label is deprecated and will not be possible anymore in a future version of sage-flatsurf. Explicitly specify a new label, e.g., new_label='split'."
        )
        ...
        if n > 3:
            for i in range(n):
                ...
                if wedge_product(e1, e2) != 0:
                    return SplitPolygonsMapping(
                        s, l, i, (i + 2) % n, new_label=new_label
                    )
            raise ValueError(
